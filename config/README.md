...
`config/default.json` 是随客户端发布的内置默认配置，必须保留。Web 在构建时导入；
远端请求失败或缺少有效字段时使用内置值。桌面端已没有社群、反馈入口，不读取该文件。

## 帮助配置来源

Web 端社群入口请求当前 endpoint 的 `GET /api/v1/client/configs`，
...
请求携带 `app_version`，省略可选平台参数。成功响应仅做 1 小时内存缓存，
请求使用 `cache: no-store`，失败不缓存。
