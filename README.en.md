...
| Web / ZxCode CLI distribution | Terminal and browser workspace; packages the TUI, Web client, backend, and Agent together  | `pnpm dev:web`                 |
| Agent CLI                     | The `zxcode` terminal interface, which also provides the Agent runtime for Desktop and Web | `pnpm --filter @zcode/cli dev` |
...
`pnpm bootstrap` installs workspace dependencies, refreshes the built-in provider configuration snapshot, prepares local desktop runtime assets, and runs `build:bootstrap`. Refreshing the provider configuration requires network access; on failure the repository snapshot is kept and setup continues.
...
| `pnpm update:builtin-provider-config`         | Refresh the built-in provider configuration snapshot alone (bootstrap runs it too)                                                  |
| `node scripts/fetch-official-marketplace.mjs` | Refresh the offline official plugin marketplace snapshot and icons alone                                                            |
...
Set `ZXCODE_DATA_BASE_DIR` to use a separate development data directory. For example, on macOS / Linux:

```bash
ZXCODE_DATA_BASE_DIR="$HOME/.zxcode-dev-home" pnpm dev:desktop:test
```

### Mobile Remote Control (Self-Hosted Relay)

Phone browsers reach existing desktop sessions through a self-hosted public relay ([packages/relay/](packages/relay/)): the desktop main process opens an outbound connection to the relay and issues one-time authorization links, and phones control the session through such a link. The relay only handles authentication, tickets, heartbeats, and byte forwarding — it holds no task queue, snapshot, or other business state.

```bash
# Build the phone pages (the relay serves the packages/web build output)
...
docker build -f packages/relay/Dockerfile -t zxcode-relay ./packages/relay
```

In the desktop app, open Settings → Mobile Remote, enter the relay URL and an optional deployment token, enable the switch, then generate a phone authorization link and open or scan it on the phone. Environment variables, TLS reverse-proxy configuration, data boundaries, and test commands are documented in [packages/relay/README.md](packages/relay/README.md).
...
### ZxCode CLI distribution

The command-line distribution includes the TUI, Web client, and Agent behind one `zxcode` command. With no arguments it starts the TUI; a leading `--web` starts Web mode; all other arguments go to the existing Agent CLI. Both modes run locally without Electron.
...
zxcode --web --workspace /path/to/project --port 3030 --no-open
...
When starting the general Web service's HTTP entry directly, configure API/WebSocket authentication with `ZXCODE_SERVER_AUTH_TOKEN`. When creating the service programmatically, use the `authToken` option.

See Packaging below for build instructions. `pnpm build:zcode` only creates the distribution; it does not replace an existing `zxcode` on `PATH`. If the command still points to an older installation or another checkout, check it with `command -v zxcode` on macOS / Linux or `where.exe zxcode` on Windows.
...
node apps/zcode-cli/packages/cli/dist/zxcode.cjs --help
```

This entry runs the Agent CLI directly and does not handle the distribution's `--web` switch. Use `pnpm dev:web` for Web development, or the extracted `bin/zxcode.mjs` shown below to test the unified command.
...
| Setting                               | Purpose                                                                                 |
| ------------------------------------- | --------------------------------------------------------------------------------------- |
| `ZXCODE_DATA_BASE_DIR`                | Base directory for application data, stored under its `.zxcode/` subdirectory           |
| `ZXCODE_SERVER_WORKSPACE`             | Workspace path for the Web backend                                                      |
| `ZXCODE_BUILTIN_PROVIDER_CONFIG_FILE` | Path to a local provider configuration file; uses the built-in configuration when unset |
| `ZXCODE_DIST_BASE_URL`                | Download base URL used by the CLI distribution installer                                |
...
Before packaging, set the download base URL with `ZXCODE_DIST_BASE_URL` in `.env`, `.env.local`, or the process environment, or pass it through `--base-url`. The URL below is a placeholder; replace it with your hosting URL when publishing:

```bash
pnpm build:zcode --base-url https://downloads.example.com/zcode/

# When ZXCODE_DIST_BASE_URL is already configured
pnpm build:zcode
...
Upload the entire directory to the configured download base URL. The installer downloads the runtime package from that URL, installs it to `~/.zxcode/runtime` by default, and creates the `zxcode` command in `~/.local/bin`. Override these directories with `ZXCODE_DIST_HOME` and `ZXCODE_DIST_BIN_DIR`, respectively.
...
node dist/zcode/debug/zcode/bin/zxcode.mjs

# Start Web mode
node dist/zcode/debug/zcode/bin/zxcode.mjs --web \
...
| `packages/relay`                                     | Self-hosted mobile remote relay; forwards bytes only                                    |
