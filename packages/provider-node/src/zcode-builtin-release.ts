...
export const ZXCODE_BUILTIN_RELEASE_SCHEMA_VERSION = 1 as const;
...
export interface ZCodeBuiltinRelease {
  readonly schemaVersion: typeof ZXCODE_BUILTIN_RELEASE_SCHEMA_VERSION;
...
const releaseSchema = z
  .object({
    schemaVersion: z.literal(ZXCODE_BUILTIN_RELEASE_SCHEMA_VERSION),
...
export function decodeZCodeBuiltinRelease(input: unknown): ZCodeBuiltinRelease {
  ...
  if (providers.has(RETIRED_ZAPI_PROVIDER_ID)) {
    throw new Error(`ZxCode Built-in Release 包含已退出的 Provider: ${RETIRED_ZAPI_PROVIDER_ID}`);
  }
  return Object.freeze({
    schemaVersion: ZXCODE_BUILTIN_RELEASE_SCHEMA_VERSION,
