...
import { ZXCODE_AGENT_PROVIDER, ZXCODE_AGENT_PROVIDER_LABEL } from "./zcode-agent-policy.js";
import type {
  ZCodeConfigOption,
...
export const botCurrentOptionsSchema = z
  .object({
    ...
    // 兼容旧 bot-config.json；CLI provider 现在统一由 ZxCode Protocol 侧配置决定。
    cli: z.literal(ZXCODE_AGENT_PROVIDER).optional(),
  })
  .strict();

export const botDraftOptionsSchema = z
  .object({
    provider: z.literal(ZXCODE_AGENT_PROVIDER),
    modelSelection: modelSelectionSchema.optional(),
    mode: z.string().min(1).optional(),
  })
  .strict();
...
export const BOT_ZCODE_PROVIDER_OPTIONS: Array<{
  id: ZCodeProvider;
  label: string;
}> = [{ id: ZXCODE_AGENT_PROVIDER, label: ZXCODE_AGENT_PROVIDER_LABEL }];
