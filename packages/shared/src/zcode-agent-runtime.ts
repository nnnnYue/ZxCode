...
export interface ZCodeAgentRuntimeDescriptor {
  ...
  /**
   * 桌面端把 agent 的 JS bundle（zxcode.cjs）打进 resources/glm，由 app 内置的 Electron Node runtime
...
export const ZXCODE_AGENT_RUNTIME: ZCodeAgentRuntimeDescriptor = {
  binaryKind: "native-binary",
  binaryEnvVar: "GLM_BINARY_PATH",
  bundledResourceDir: "glm",
  version: "0.13.3",
  spawnArgs: ["app-server", "--stdio"],
  nativeConfigDir: ".zxcode/cli",
  nativeConfigFileName: "config.json",
  missingBinaryMessage:
    "[ZxCode Agent] glm binary 未找到，请设置 GLM_BINARY_PATH 或先准备 GLM 运行时资源",
  resolveEntrySegments: (platform) => [resolvePlatformBinaryName("zxcode-agent", platform)],
  nodeBundleEntryFile: "zxcode.cjs",
