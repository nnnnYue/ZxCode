export const ZXCODE_RUNTIME_ENV_KEY = "ZXCODE_RUNTIME_ENV";
export const ZXCODE_HTTP_PROXY_ENV_KEY = "ZXCODE_HTTP_PROXY";
export const ZXCODE_NO_PROXY_ENV_KEY = "ZXCODE_NO_PROXY";
/** Desktop Host 只向 desktop-attached remote server 传递一次的网络配置。 */
export const ZXCODE_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY =
  "ZXCODE_REMOTE_RUNTIME_NETWORK_AUTHORITY";
export const ZXCODE_REMOTE_HTTP_PROXY_ENV_KEY = "ZXCODE_REMOTE_HTTP_PROXY";
export const ZXCODE_REMOTE_NO_PROXY_ENV_KEY = "ZXCODE_REMOTE_NO_PROXY";
export const ZXCODE_AGENT_CA_CERT_ENV_KEY = "ZXCODE_AGENT_CA_CERT";
export const ZXCODE_TOOL_ENV_PASSTHROUGH_ENV_KEY = "ZXCODE_TOOL_ENV_PASSTHROUGH_JSON";
/** Desktop Main 将服务端裁决的单功能灰度结果传给 Local/Remote Host。 */
export const ZXCODE_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV = "ZXCODE_DESKTOP_CONTEXT_PROMPT_ENABLED";
export const ZXCODE_CUA_PRODUCT_HELPER_ENV_KEY = "ZXCODE_CUA_PRODUCT_HELPER";
export const ZXCODE_CUA_BROKER_SOCKET_ENV_KEY = "ZXCODE_CUA_PERMISSION_BROKER_SOCKET";
/** Shared node_repl host marker; unlike the broker bearer values it is not a secret. */
export const ZXCODE_CUA_NODE_REPL_HOST_ENV_KEY = "ZXCODE_CUA_NODE_REPL_HOST";
// One-knob local-development bundle. Setting ZXCODE_CUA_DEV_MODE implies the internal feature
...
export const ZXCODE_CUA_DEV_MODE_ENV_KEY = "ZXCODE_CUA_DEV_MODE";
...
export function isCuaDevModeRequested(env: EnvRecord = process.env): boolean {
  const explicit = env[ZXCODE_CUA_DEV_MODE_ENV_KEY]?.trim().toLowerCase();
...
export function isZCodeCuaInternalFeatureEnabled(env: EnvRecord = process.env): boolean {
  ...
  const explicit = env[ZXCODE_CUA_PRODUCT_HELPER_ENV_KEY]?.trim().toLowerCase();
...
const SANITIZED_RUNTIME_ENV_KEYS = [
  ...
  "ZXCODE_CUA_PERMISSION_BROKER_TOKEN",
  "ZXCODE_CUA_PERMISSION_BROKER_REFRESH_MARKER",
  "ZXCODE_CUA_PLUGIN_AUTHORITY",
] as const;
...
export function resolveZCodeRuntimeEnv(
  ...
  return normalizeZCodeRuntimeEnv(env[ZXCODE_RUNTIME_ENV_KEY]) ?? fallback;
}
...
export const ZXCODE_CUA_PLUGIN_AUTHORITY_ENV_KEY = "ZXCODE_CUA_PLUGIN_AUTHORITY";
...
// 解析全局 ~/.zxcode/cli/config.json 里的 `zcode-cua` server 之前就会先 sanitize process.env，导致
...
function captureZCodeCuaBrokerCredentials(env: Record<string, string | undefined>): void {
  const socket = env[ZXCODE_CUA_BROKER_SOCKET_ENV_KEY]?.trim();
  const pluginAuthority = env[ZXCODE_CUA_PLUGIN_AUTHORITY_ENV_KEY]?.trim();
  const refreshMarker = env["ZXCODE_CUA_PERMISSION_BROKER_REFRESH_MARKER"]?.trim();
...
export function readZCodeToolEnvPassthroughEnv(env: EnvRecord): Record<string, string> {
  const raw = env[ZXCODE_TOOL_ENV_PASSTHROUGH_ENV_KEY];
  if (!raw) {
    return {};
  }
...
function stringifyZCodeToolEnvPassthroughEnv(
  ...
  return {
    [ZXCODE_TOOL_ENV_PASSTHROUGH_ENV_KEY]: JSON.stringify(Object.fromEntries(entries)),
  };
}
