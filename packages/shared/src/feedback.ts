...
export type FeedbackTicketFramework = "zxcode-agent";
...
export interface FeedbackDeviceInfo {
  ...
  osVersion?: string;
  osArch?: string;
  /** 提交反馈时的当前 Agent。单 ZxCode Agent 模式下固定为 ZxCode Agent。 */
  agentProvider?: string;
  /** 提交反馈时归一到反馈平台的框架标识，当前固定为 zxcode-agent。 */
...
export const DEFAULT_FEEDBACK_TICKET_FRAMEWORK: FeedbackTicketFramework = "zxcode-agent";

export const FEEDBACK_TICKET_FRAMEWORK_OPTIONS: {
  value: FeedbackTicketFramework;
  label: string;
}[] = [{ value: "zxcode-agent", label: "ZxCode Agent" }];
