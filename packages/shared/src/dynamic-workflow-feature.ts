 ...
 */
export const ZXCODE_DYNAMIC_WORKFLOW_MODE_ENV = "ZXCODE_DYNAMIC_WORKFLOW_MODE";
 ...
 * 服务端撤掉 key 等于关闭，不能沿用旧快照（单功能灰度的统一裁决语义）。
 */
export function resolveDynamicWorkflowClientConfig(input: {
  remote: unknown;
  env?: Record<string, string | undefined>;
}): DynamicWorkflowClientConfig {
  const override = normalizeDynamicWorkflowMode(input.env?.[ZXCODE_DYNAMIC_WORKFLOW_MODE_ENV]);
