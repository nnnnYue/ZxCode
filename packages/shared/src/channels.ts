...
export const PlatformChannels = {
  /** 打开系统目录选择框 */
  SelectDirectory: "zxcode:select-directory",
  /** 打开系统文件选择框 */
  SelectFile: "zxcode:select-file",
  /** 打开系统多文件选择框 */
  SelectFiles: "zxcode:select-files",
  /** Renderer → Main：写入宿主 ~/.zxcode 临时文本附件 */
  CreateTempTextAttachment: "zxcode:create-temp-text-attachment",
  /** Renderer → Main：通过原生另存为对话框保存文件 */
  SaveFile: "zxcode:save-file",
  /** Renderer → Main：用 Chromium 打印引擎把当前页面 print 媒体版面导出为 PDF */
  PrintToPdf: "zxcode:print-to-pdf",
  /** Main → Renderer：转发远程连接过程日志 */
  RemoteConnectionLog: "zxcode:remote-connection-log",
  /** Main → Renderer：远程 workspace session 已关闭 */
  RemoteSessionClosed: "zxcode:remote-session-closed",
  /** Main → Renderer：Bot 已触发远端 workspace 重连成功 */
  BotRemoteWorkspaceReconnected: "zxcode:bot-remote-workspace-reconnected",
  /** 检查目录是否已在其他窗口打开，如果是则激活该窗口 */
  ActivateOrSetWorkspace: "zxcode:activate-or-set-workspace",
  /** 建立 SSH 远程连接 */
  ConnectRemote: "zxcode:connect-remote",
  /** 取消当前窗口正在进行中的远程连接 */
  CancelPendingRemoteConnection: "zxcode:cancel-pending-remote-connection",
  /** Renderer → Main：绑定远程 logical session 的 canonical workspace context */
  BindRemoteWorkspaceSessionContext: "zxcode:bind-remote-workspace-session-context",
  /** 释放当前窗口里的远程 session */
  DisposeRemoteSession: "zxcode:dispose-remote-session",
  /** Renderer → Main：检查本机 Docker daemon 是否可用 */
  IsDockerAvailable: "zxcode:is-docker-available",
  /** Renderer → Main：列出本机可用的 WSL 发行版 */
  ListWSLDistros: "zxcode:list-wsl-distros",
  /** Renderer → Main：列出当前可连接的 Docker 容器 */
  ListDockerContainers: "zxcode:list-docker-containers",
  /** Renderer → Main：列出 SSH config 里可用于快速填表的 alias */
  ListSSHConfigAliases: "zxcode:list-ssh-config-aliases",
  /** Renderer → Main：从用户目录加载 CLI MCP 配置 */
  LoadMcpFromUserDirectory: "zxcode:load-mcp-from-user-directory",
  /** Renderer → Main：保存 CLI MCP 配置到用户目录 */
  SaveMcpToUserDirectory: "zxcode:save-mcp-to-user-directory",
  /** Renderer 日志转发到 main 进程统一存储 */
  Log: "zxcode:log",
  /** Renderer → Main：同步当前窗口所有 tab 的 workspace 路径 */
  SyncWindowTabs: "zxcode:sync-window-tabs",
  /** Renderer → Main：同步当前窗口的未读 task 数 */
  SyncWindowUnreadCount: "zxcode:sync-window-unread-count",
  /** Renderer → Main：当前窗口 active task，只更新 Main 的临时焦点映射。 */
  SyncActiveTaskSession: "zxcode:sync-active-task-session",
  /** Renderer → Main：同步 main 进程需即时感知的应用设置 */
  SyncAppSettings: "zxcode:sync-app-settings",
  /** Renderer → Main：上报当前窗口 active workspace 三元组（手机远控 presence 用） */
  SyncWindowWorkspace: "zxcode:sync-window-workspace",
  /** Renderer → Main：查询自部署手机远控 relay 的连接状态 */
  GetMobileRelayStatus: "zxcode:get-mobile-relay-status",
  /** Renderer → Main：请求 relay 签发一次性手机远控授权链接 */
  RequestMobileRelayGrant: "zxcode:request-mobile-relay-grant",
  /** Renderer → Main：快捷键设置页录制态开关；true = main 暂时摘除可配置菜单 accelerator */
  SetShortcutRecordingActive: "zxcode:set-shortcut-recording-active",
  /** Main → Renderer：聚焦到指定 workspace 路径的 tab */
  FocusTab: "zxcode:focus-tab",
  /** Main → Renderer：菜单触发新建 tab */
  NewTab: "zxcode:new-tab",
  /** Main → Renderer：菜单或快捷键请求关闭当前上下文 */
  CloseActiveContextRequest: "zxcode:close-active-context-request",
  /** Main → Renderer：内置 webview 请求打开新的浏览器 tab */
  OpenBrowserUrl: "zxcode:open-browser-url",
  /** Main → Renderer：agent 首次 browser 命令建好受控 view，通知 renderer 自动开 browser-use tab */
  BrowserViewReady: "zxcode:browser-view-ready",
  /** Main → Renderer：agent 正在操作某个 browser-use tab，renderer 临时显示状态图标 */
  BrowserViewOperation: "zxcode:browser-view-operation",
  /** Main → Renderer：browser visibility capability 显示/隐藏 IAB 右侧面板 */
  BrowserViewVisibility: "zxcode:browser-view-visibility",
  /** Main → Renderer：Agent 设置/重置目标 tab viewport */
  BrowserViewViewportChanged: "zxcode:browser-view-viewport-changed",
  /** Main → Renderer：截图前请求 owner renderer 准备后台 guest 合成表面。 */
  BrowserViewScreenshotSurfacePrepare: "zxcode:browser-view-screenshot-surface-prepare",
  /** Renderer → Main：目标 guest 连续两个 animation frame 的 viewport 已稳定。 */
  BrowserViewScreenshotSurfaceReady: "zxcode:browser-view-screenshot-surface-ready",
  /** Main → Renderer：截图结束或准备失败，释放临时后台合成层。 */
  BrowserViewScreenshotSurfaceRelease: "zxcode:browser-view-screenshot-surface-release",
  /** Main → Renderer：agent close 命令 detach 受控 guest 后，通知 renderer 卸载对应 tab */
  BrowserViewCloseTab: "zxcode:browser-view-close-tab",
  /** Main → Renderer：预算淘汰时卸载 guest，但保留 logical tab shell。 */
  BrowserViewSuspend: "zxcode:browser-view-suspend",
  /** Main → Renderer：为 suspended shell 重新挂载 guest。 */
  BrowserViewRestore: "zxcode:browser-view-restore",
  /** Main → Renderer：菜单触发新建任务 */
  NewTask: "zxcode:new-task",
  /** Main → Renderer：菜单触发打开工作区 */
  OpenWorkspace: "zxcode:open-workspace",
  /** Main → Renderer：deep link 直接打开指定本地工作区目录 */
  OpenWorkspacePath: "zxcode:open-workspace-path",
  /** Main → Renderer：窗口全屏状态变化 */
  WindowFullscreenChanged: "zxcode:window-fullscreen-changed",
  /** Renderer → Main：读取窗口最大化状态与系统原生圆角能力 */
  GetDesktopWindowChromeState: "zxcode:get-desktop-window-chrome-state",
  /** Main → Renderer：窗口最大化状态与系统原生圆角能力变化 */
  DesktopWindowChromeStateChanged: "zxcode:desktop-window-chrome-state-changed",
  /** Main → Renderer：原生窗口控制区安全边距变化 */
  WindowControlsOverlayChanged: "zxcode:window-controls-overlay-changed",
  /** Preload → Main：preload 已同步读到当前窗口控制区安全边距 */
  WindowControlsOverlayReady: "zxcode:window-controls-overlay-ready",
  /** 获取资源管理器快照（CPU / 内存，按基础服务、内置插件、社区插件归类） */
  GetResourceUsageSnapshot: "zxcode:get-resource-usage-snapshot",
  SetResourceUsageSamplingActive: "zxcode:set-resource-usage-sampling-active",
  /** 打开资源管理器窗口（其他窗口触发） */
  OpenResourceManager: "zxcode:open-resource-manager",
  /** 资源管理器「存储」tab：开始扫描本机 .zxcode 占用（main 持有 StorageService，Worker 线程遍历） */
  StorageStartScan: "zxcode:storage-start-scan",
  /** 资源管理器「存储」tab：取消扫描 */
  StorageCancelScan: "zxcode:storage-cancel-scan",
  /** 资源管理器「存储」tab：读取最近一次快照 */
  StorageGetSnapshot: "zxcode:storage-get-snapshot",
  /** 资源管理器「存储」tab：按类别清理 */
  StorageClean: "zxcode:storage-clean",
  ...
  OpenExternalFile: "zxcode:open-external-file",
  /** Renderer → Main：打开 ZxCode Computer Use 权限引导 */
  OpenCuaPermissionOnboarding: "zxcode:open-cua-permission-onboarding",
  /** Renderer → Main：取消当前 renderer 发起的一次权限引导 participant */
  CancelCuaPermissionOnboarding: "zxcode:cancel-cua-permission-onboarding",
  /**
   * Renderer → Main：预热并缓存已验证的 Helper 路径 + bundle 指纹。
   * 必须在拖拽浮窗挂载时调用 —— dragstart 链路里不允许任何异步 I/O。
   */
  PrepareCuaHelperPermissionDrag: "zxcode:prepare-cua-helper-permission-drag",
  /** Renderer → Main：把已验证的 Helper.app 同步拖出到 macOS 权限列表 */
  StartCuaHelperPermissionDrag: "zxcode:start-cua-helper-permission-drag",
  /**
   ...
   * 立刻消失可能打断正在进行的拖拽。
   */
  NotifyCuaHelperPermissionDragEnded: "zxcode:notify-cua-helper-permission-drag-ended",
  /** Main → Renderer：转发支付 deep link URL */
  PaymentCallback: "zxcode:payment-callback",
  /** Renderer → Main：renderer 已就绪，可接收缓存的 deep link */
  RendererReady: "zxcode:renderer-ready",
  /** Renderer → Main：主窗口 renderer 每 60 秒的 heap 读数，单向 send，不需要回执。 */
  ReportRendererHeapSample: "zxcode:report-renderer-heap-sample",
  /** Renderer → Main：触发任务完成/失败的系统通知 */
  ShowTaskNotification: "zxcode:show-task-notification",
  /** Main → Preload：通知 renderer 播放任务通知提示音 */
  TaskNotificationSound: "zxcode:task-notification-sound",
  /** Main → Preload：用户点击了系统通知，携带 taskId 让 renderer 跳转到对应任务 */
  TaskNotificationClick: "zxcode:task-notification-click",
  /** Renderer → Main：导出日志（打包 ~/.zxcode/v2 及外部 agent 日志为 zip 并在 Finder 中显示） */
  ExportLogs: "zxcode:export-logs",
  /** Renderer → Main：截取当前窗口作为反馈附件 */
  CaptureWindowScreenshot: "zxcode:capture-window-screenshot",
  /**
   ...
   */
  BrowserViewAttachGuest: "zxcode:browser-view-attach-guest",
  /** Renderer → Main：重建 `<webview>` 前主动断开旧 guest 的 CDP。 */
  BrowserViewDetachGuest: "zxcode:browser-view-detach-guest",
  /** Renderer → Main：用户显式关闭 Browser tab。 */
  BrowserViewCloseTabFromRenderer: "zxcode:browser-view-close-tab-from-renderer",
  /** Renderer → Main：上报 Browser tab residency/display facts。 */
  BrowserViewReportResidency: "zxcode:browser-view-report-residency",
  /** Renderer → Main：指定 generation 的 guest 已卸载。 */
  BrowserViewSuspendReady: "zxcode:browser-view-suspend-ready",
  /** Renderer → Main：用户访问 suspended tab，请求恢复。 */
  BrowserViewEnsureResident: "zxcode:browser-view-ensure-resident",
  /** Renderer → Main：读取 workspace/task 的持久化 logical shells。 */
  BrowserViewRestoreTabs: "zxcode:browser-view-restore-tabs",
  /** Renderer → Main：自由尺寸拖拽/开关回写目标 guest viewport */
  BrowserViewUpdateViewport: "zxcode:browser-view-update-viewport",
  /** Embedded Browser preload → Main：在网页原生 Dialog 创建前同步请求可信系统框。 */
  EmbeddedBrowserJavaScriptDialog: "zxcode:embedded-browser-javascript-dialog",
  /** Renderer → Main：把自动发现的本机 Chrome Profile 数据一次性导入内置浏览器分区。 */
  ImportChromeBrowserData: "zxcode:import-chrome-browser-data",
  /** Renderer → Main：清理内置浏览器缓存或全部站点数据。 */
  ClearEmbeddedBrowserData: "zxcode:clear-embedded-browser-data",
  /** Renderer → Main：查询桌面端正在运行的会话数量 */
  GetDesktopSessionActivity: "zxcode:get-desktop-session-activity",
  /** Renderer → Main：读取当前窗口页面缩放档位 */
  GetDesktopZoomLevel: "zxcode:get-desktop-zoom-level",
  /** Main → Renderer：当前窗口页面缩放档位变化 */
  DesktopZoomLevelChanged: "zxcode:desktop-zoom-level-changed",
  /** Renderer → Main：读取开发态 stdio tap proxy 开关状态 */
  GetZCodeStdioTapDevState: "zxcode:get-zcode-stdio-tap-dev-state",
  /** Main → Renderer：本地 setting.json 已由 main 进程更新 */
  SettingsChanged: "zxcode:settings-changed",
  /** Main → Renderer：应用语言已切换 */
  ApplicationLocaleChanged: "zxcode:application-locale-changed",
  /** Renderer → Main：读取宿主系统语言 */
  GetSystemLocale: "zxcode:get-system-locale",
  /** Renderer → Main：获取系统中已安装的编辑器/终端列表（含图标） */
  GetInstalledEditors: "zxcode:get-installed-editors",
  ...
  SetApplicationLocale: "zxcode:set-application-locale",
  /** Renderer → Main：同步标题栏亮暗色，用于原生窗口控制按钮配色 */
  SetTitleBarTheme: "zxcode:set-title-bar-theme",
  /** Renderer → Main：迁移旧版 Common MCP 配置 */
  MigrateLegacyCommonMcp: "zxcode:migrate-legacy-common-mcp",
  /** Renderer → Main：获取当前设备的稳定标识符（deviceMid） */
  GetDeviceId: "zxcode:get-device-id",
} as const;
...
export const EmbeddedBrowserWebviewChannels = {
  ...
  WheelBoundary: "zxcode:embedded-browser-wheel-boundary",
} as const;
...
export const InternalChannels = {
  DatabaseStartupState: "zxcode:database-startup-state",
  DatabaseStartupControl: "zxcode:database-startup-control",
  /** main → renderer 转发 MessagePort（通过 webContents.postMessage） */
  ServicePort: "zxcode:service-port",
  /** main → renderer 转发窗口 Host 的 scoped MessagePort */
  ScopedServicePort: "zxcode:scoped-service-port",
  /** renderer → main：scoped MessagePort 已注册，可安全切换 attachment */
  ScopedServicePortReady: "zxcode:scoped-service-port-ready",
  /** preload → renderer：主进程已确认系统通知展示，renderer 可播放提示音 */
  TaskNotificationSound: "zxcode:task-notification-sound",
} as const;

/** @deprecated `/ws` 已忽略该头；保留常量仅供旧客户端兼容。 */
export const ZXCODE_RPC_CLIENT_MODE_HEADER = "x-zxcode-rpc-client-mode";
/** desktop 先经受保护 HTTP endpoint 申请，再在 `/ws/host` 握手时一次性消费。 */
export const ZXCODE_RPC_HOST_CAPABILITY_HEADER = "x-zxcode-rpc-host-capability";
...
export const HostMessageTypes = {
  ...
  /** Main → Host：全局前台 ZxCode 窗口派生的 producer focus fact。 */
...
export const HostResponseTypes = {
  ...
  /** host → main：声明一个 ZxCode Agent session 当前归属该 host */
