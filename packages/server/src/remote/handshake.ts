...
import { ZXCODE_VERSION, formatZodError, helloMessageSchema } from "@zcode/shared";
 ...
 * 1. Read lines from stdout until we find a zxcode-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a zxcode-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no zxcode-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "zxcode-hello"
            ) {
              rejectWithDiagnostics(`Invalid zxcode-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
