...
export function PluginStorePage({
  ...
  // 目录自动刷新（Catalog Auto-Refresh）已随官方市场离线化移除：目录在编译期随包
  // 分发（resources/official-marketplace + bundled seed），运行时不存在官方 CDN fetch
  // 路径，进入商店页不再发起任何目录刷新。顶栏手动刷新仍可用，但 adapters 侧对官方
  // id 一律短路，只更新用户显式添加的自定义市场。
  ...
  // 顶栏刷新 = updateMarketplace(null)：重拉用户显式添加的自定义市场（git/url 源），
  // 官方市场对刷新一律短路、只读随包快照。操作完成后按更新徽标数量给完成提示。
