...
export function shouldMirrorBotTaskStreamToStore(params: {
  ...
  // Bugfix: 远端 Bot task 的 stream 来自 bot runtime host，不一定会被当前 ChatView 的 ZxCode Agent stream 订阅收到。
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      ...
      if (provider && shouldSyncVisibleTaskConfig) {
        // Bugfix: /model、/mode 可以从第三方 Bot 修改当前 task 的真实 ZxCode Agent 状态。
      ...
      if (refresh.event === "permission_request" && refresh.permissionRequest) {
        ...
        // 当前 UI 窗口不一定有同一条 ZxCode Agent stream 订阅，必须把新的 elicitation_request 显式写回 store。
        ...
        // Bugfix: Bot 代用户提交 AskUserQuestion 时，当前 UI 窗口不一定能收到 ZxCode Agent stream 的
