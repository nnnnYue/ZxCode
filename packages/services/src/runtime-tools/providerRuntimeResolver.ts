...
import { ZXCODE_AGENT_RUNTIME } from "@zcode/shared";
...
function resolvePlatformScopedBundledAgentRoots(moduleDir?: string): Array<string | null> {
  ...
  return [
    ...
    // ZxCode Agent 资源可能位于桌面包或仓库根的 bundled-agents/<platform>。
...
export function findZCodeAgentRuntimeBinary(): string | null {
  ...
  // import.meta.dirname 在打包后的 CJS bundle（zxcode-server.cjs）中是 undefined，
  ...
  const candidates = [
    ...
    resolvePath(homedir(), ".zxcode", "server", "agents", ...resourceSegments),
...
/**
 * 查找 agent 的 JS bundle（resources/glm/zxcode.cjs）。
