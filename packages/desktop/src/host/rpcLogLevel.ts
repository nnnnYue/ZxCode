import { ZXCODE_AGENT_PROVIDER_NOT_READY_CODE } from "@zcode/shared";
...
export function resolveRpcLogLevel(message: string, ...args: unknown[]): RpcLogLevel {
  ...
  if (message.startsWith("[rpc:call] zxcode-agent.backgroundBashOutputV4 OK (")) return "debug";
  ...
  if (
    ...
    args.some(
      (arg) =>
        ...
        arg !== null &&
        "code" in arg &&
        (arg as { code?: unknown }).code === ZXCODE_AGENT_PROVIDER_NOT_READY_CODE,
    )
  ) {
    return "info";
  }
