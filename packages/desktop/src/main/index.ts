...
import { createDesktopRelayClient, type DesktopRelayClientHandle } from "./desktopRelayClient.js";
...
// 内存诊断计数器：周期性写主日志，用于观察关窗后 defaultSession 是否残留监听。
...
// 手机远控 relay 的窗口 active workspace 镜像：renderer 经 SyncWindowWorkspace 上报，
// Main 不回查 renderer；grant target 校验与 presence 都以该映射为准。
const windowWorkspaceInfoMap = new Map<
  number,
  { workspacePath: string; workspaceIdentity?: string; workspaceKey: string }
>();
// 自部署手机远控 relay 客户端（main 单例）：设置驱动启停，attach 走窗口 Host/远程 session。
const mobileRelayClient: DesktopRelayClientHandle = createDesktopRelayClient({
  logger,
  deviceId: deviceMid,
  attachLocalWorkspace: (windowId) => {
    // 复刻 renderer reload 重连先例（desktopWindowLifecycle），但 clientMode 用
    // web-remote-replayable：手机端必须走 snapshot+delta 重放语义，不复用 continuous。
    const win = BrowserWindow.fromId(windowId);
    if (!win || win.isDestroyed()) {
      throw new Error(`未找到窗口 windowId=${windowId}`);
    }
    const child = windowHostProcessMap.get(win.webContents.id);
    if (!child || child.pid == null) {
      throw new Error(`未找到窗口 Local Host，windowId=${windowId}`);
    }
  ...
  listWindowWorkspaces: () =>
    Array.from(windowWorkspaceInfoMap.entries()).map(([windowId, info]) => ({
      windowId,
      ...info,
    })),
  listRemoteWorkspaceSessions: remoteSessionManager.listRemoteWorkspaceSessions,
});
...
function syncImmediateAppSettings(patch: Partial<AppSettings>) {
  ...
  if (patch.mobileRelay !== undefined) {
    // 手机远控 relay 启停/换服务器即时生效：客户端内部处理重连与断开。
    mobileRelayClient.applySettings(patch.mobileRelay);
  }
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  if (selection.upgraded && (hasPreparedAppQuit || appQuitPreparationInFlight)) {
    logger.warn(
      `[app-quit] upgraded shutdown joined an existing barrier (${reason}); existing timers keep their original budget`,
    );
  }
  ...
  // relay 控制连接先断开：退出期不再接受新的手机 attach。
  void mobileRelayClient.dispose();
...
app.whenReady().then(async () => {
  ...
  // 手机远控 relay：启动读一次设置决定是否出站连接（后续变更走 SyncAppSettings）。
  mobileRelayClient.applySettings(bootstrapSettings?.mobileRelay);
  ...
  registerPlatformIpcHandlers({
    ...
    syncWindowWorkspace: (windowId, payload) => {
      windowWorkspaceInfoMap.set(windowId, payload);
      mobileRelayClient.pushPresence();
    },
    getMobileRelayStatus: () => mobileRelayClient.getStatus(),
    requestMobileRelayGrant: (payload) => mobileRelayClient.requestGrant(payload),
...
app.on("browser-window-created", (_, win) => {
  ...
  win.on("closed", () => {
    ...
    windowWorkspaceInfoMap.delete(win.id);
