...
const ABOUT_APPLICATION_NAME = "ZxCode Desktop App";
...
export function createAboutSnapshot(options: AboutSnapshotOptions = {}): AboutSnapshot {
  ...
  return {
    appVersion: normalizeValue(options.appVersion ?? buildMetadata?.appVersion ?? ZXCODE_VERSION),
    buildCommitId: normalizeValue(buildMetadata?.buildCommitId ?? ZXCODE_COMMIT),
    buildTime: normalizeValue(buildMetadata?.buildTime ?? ZXCODE_BUILD_TIME),
    environment: normalizeValue(options.environment ?? ZXCODE_ENV),
