...
export function resolveZCodeBuiltinProviderConfigFilePath(options?: {
  ...
  const explicitPath = (options?.env ?? process.env)["ZXCODE_BUILTIN_PROVIDER_CONFIG_FILE"]?.trim();
  if (explicitPath) return explicitPath;
  if (options?.isPackaged ?? app.isPackaged) {
    return join(
      options?.resourcesPath ?? process.resourcesPath,
      "config/provider/zxcode-builtin.json",
    );
  }
  // 开发态与打包共用唯一线上配置源。
  const filename = "zxcode-builtin.json";
