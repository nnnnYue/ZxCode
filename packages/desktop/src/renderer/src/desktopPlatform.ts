...
export function createDesktopPlatform(options: {
  ...
  return {
    ...
    syncWindowWorkspace: (payload) => window.zcode.syncWindowWorkspace?.(payload),
    getMobileRelayStatus: () => window.zcode.getMobileRelayStatus?.(),
    requestMobileRelayGrant: (payload) => window.zcode.requestMobileRelayGrant?.(payload),
