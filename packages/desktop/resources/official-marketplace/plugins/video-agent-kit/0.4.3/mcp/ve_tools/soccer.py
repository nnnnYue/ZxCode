...
TIERS_RATE = {0: 12, 1: 20, 2: 28}          # cloud_tts speech_rate
TIERS_LUFS = {0: -17.5, 1: -16.5, 2: -14.5}
CHARS_PER_SEC = {0: 4.9, 1: 5.1, 2: 5.3}
DEFAULT_TTS_PROVIDER = "cloud_tts"
DEFAULT_SOCCER_TTS_VOICE = "zh_male_liufei_uranus_bigtts"
...
def _synth_cloud(text: str, voice: str, tier: int, dst: Path, ctx: RunContext) -> str | None:
    ...
    import time
    for k in range(4):
        r = tts_generate({"text": text, "preferred_provider": "cloud_tts", "speaker": voice,
                          "speech_rate": TIERS_RATE[tier], "pitch_rate": 0,
                          "output_path": str(dst)}, ctx)
        if not r.text.startswith("[ERROR]"):
            return None
        time.sleep(8 * (2 ** k))
    return f"cloud_tts synth failed: {r.text[:120]}"
...
def soccer_tts(args: dict, ctx: RunContext) -> ToolResult:
    ...
    if provider in ("auto", "cloud", "cloud_tts"):
