---
description: 用 ZxCode 内置 Browser Use WebView 把网站 URL 的动效录成 WebM 再用 ffmpeg 转 MP4 — 踩点→编排动作→录制→转码→审片
...
使用 ZxCode 官方 `control-browser` skill 打开和踩点页面，编排内置 recording action DSL，
通过 `tab.recording.start()` + `status(..., {outputPath})` 产出
`recordings/<name>.webm`（ZxCode 只落 WebM），随后用 ffmpeg 转成 `recordings/<name>.mp4`
