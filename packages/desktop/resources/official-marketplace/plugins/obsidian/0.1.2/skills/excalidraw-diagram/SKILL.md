...
6. **Save the file** — into the user's vault when the vault path is known (e.g. the session already works inside a vault); otherwise ask the user where to save, falling back to the current working directory
...
#### 3. 使用 Write 工具自动保存文件
- **保存位置优先级**：用户 vault（会话中已知 vault 路径时，存入 vault 内合适目录，如已有的 Excalidraw 素材目录或当前笔记所在目录）→ 询问用户 → 当前工作目录
- vault 内路径：`{vault_directory}/[filename].md`；确实无法确定 vault 时才退回 `{current_directory}/[filename].md`
- 不硬编码任何绝对路径
