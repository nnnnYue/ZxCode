...
If not installed: ask first (Ask User, or in the conversation) and wait for
explicit approval before running `npm install -g knap` (requires Node.js 20
or later); the user may install it manually or skip. `npx knap` downloads
packages, so it needs the same approval — apply the gate in
`../setup/references/setup-preflight.md`.
