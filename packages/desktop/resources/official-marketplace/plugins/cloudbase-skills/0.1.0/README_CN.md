...
开发知识与 CloudBase MCP 服务包装成一个可直接安装的 ZxCode 插件。
...
在 ZxCode 插件管理器中安装并启用 `cloudbase-skills`，然后新建任务。请求与技能描述匹配时，ZxCode
...
本插件对 MIT 许可证下的上游项目进行再分发和 ZxCode 集成。来源与本地包装变更见
