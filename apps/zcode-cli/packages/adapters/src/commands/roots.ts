...
function commandRootsForBase(
  ...
  // 合并而不是 fallback：兼容 `.agents` 命令和原生 `.zxcode` 命令需要同时可见。
  // 同一级别 `.zxcode` 先扫描，命令同名时仍按“先到先赢”处理。
  return [
    root(join(baseDirectory, ZXCODE_DIR, COMMANDS_DIR), scope, "zxcode", nextPriority()),
