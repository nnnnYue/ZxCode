// Env Config Adapter - Parse the intentionally small ZXCODE_* environment surface.
...
const DEFAULT_PREFIX = "ZXCODE_";

/**
 * Parse ZXCODE_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.ZXCODE_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}
