...
const PLUGINS_COMMAND_USAGE = `Usage: zxcode plugins <command> [options]
...
Scopes: user (default), project. \`zxcode plugin\` is an alias of \`zxcode plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart zxcode to apply.\n`,
  );
