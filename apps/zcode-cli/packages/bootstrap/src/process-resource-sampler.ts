...
import {
  ZXCODE_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type ZCodeProcessResourceSample,
} from "@zcode/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const ZXCODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = ZXCODE_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
export function createZCodeProcessResourceSampler(
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, ZXCODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
