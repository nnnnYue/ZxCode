...
// `helperInstaller` 因此随本 bundle 进入正式包。但 `__ZXCODE_CUA_HELPER_BUILD_ID__` 此前
// **只有** packages/desktop/tsup.config.ts 注入（Electron main / app.asar），本文件的
// esbuild 调用一个 define 都没有。于是正式包里 producer 的 `ZXCODE_CUA_HELPER_BUILD_ID`
...
//
//   "Packaged ZxCode is missing its embedded Computer Use Helper build identity;
...
export const buildNodeReplHostBundle = async ({
  ...
  await build({
    ...
    bundle: true,
    define: {
      __ZXCODE_CUA_HELPER_BUILD_ID__: JSON.stringify(cuaHelperBuildId),
    },
  ...
  if (cuaHelperBuildId) {
    ...
    if (!bundled.includes(cuaHelperBuildId)) {
      throw new Error(
        `[node-repl-host] ZXCODE_CUA_HELPER_BUILD_ID=${cuaHelperBuildId} 未折叠进 ${outfile}：` +
          "__ZXCODE_CUA_HELPER_BUILD_ID__ define 没有生效，正式包的 Helper 安装会被 fail-closed 拒绝。",
      );
    }
  }
