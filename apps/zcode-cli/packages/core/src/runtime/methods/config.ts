 ...
 * workflow child 过去直接从 `appOptions` 取 `permissionBroker`，于是带着 `sess_dwf-…`去问桌面；
 * 桌面回包路径上的 `requireSession` 抛错、response 永不发出，子代理在首个模型请求前永久挂起
 * （8 个子代理、80 分钟无任何事件）。core 内建 subagent 当时靠私有 wrapper 绕开，三处装配
 * 两错一对——说明规则散落在调用点就一定会漂。
 *
