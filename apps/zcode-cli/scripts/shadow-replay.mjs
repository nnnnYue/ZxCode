// 影子重放对账（交付 / 测试基建）。
// 用途：把本机真实 CLI 库（默认 ~/.zxcode/cli/db/db.sqlite）的历史会话全量喂给
...
const sourceDbPath = args.db ?? join(homedir(), ".zxcode", "cli", "db", "db.sqlite");
const limit = args.limit ? Number(args.limit) : Infinity;

let dbPath = sourceDbPath;
if (!args["no-copy"]) {
  const tempDir = mkdtempSync(join(tmpdir(), "zxcode-shadow-replay-"));
