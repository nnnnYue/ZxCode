...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "zxcode-native-search-build-"));
  ...
  console.log("==> ZxCode native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DZXCODE_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DZXCODE_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DZXCODE_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DZXCODE_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DZXCODE_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DZXCODE_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DZXCODE_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DZXCODE_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
